...
* `touchcontrols="left,right,up,down"`: A comma-separated list of the touch controls that should be enabled on this chart if `touchControls` is set to true in `config.ini`. Available controls are `left`, `right`, `up`, `down`.
